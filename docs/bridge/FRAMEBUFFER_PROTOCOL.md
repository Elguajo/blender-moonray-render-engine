...
|---|---|
| Exact transport (shared-memory ring buffer vs. binary socket stream vs. mapped file) | Phase 04, revisited Phase 08/10 |
| Snapshot cadence for the viewport (fixed interval, sample-count threshold, adaptive) | Phase 08 |
| Pixel/channel layout on the wire (planar vs. interleaved, float32 vs. half) | Phase 04 |
