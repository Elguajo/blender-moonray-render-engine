...
PY_EXPR='import bpy,sys,platform; print("BLENDER_VERSION="+bpy.app.version_string); print("PYTHON="+sys.version.replace("\n"," ")); print("PLATFORM="+platform.platform()); from pxr import Usd,Tf; print("USD_VERSION="+str(Usd.GetVersion())); print("TF_MODULE="+str(Tf.__file__))'
if "${BLENDER}" --background --factory-startup --python-expr "${PY_EXPR}"; then
...
if "${BLENDER}" --background --factory-startup --python-expr "${CREATE_EXPR}" && [[ -s "${BLEND}" ]]; then
...
if "${BLENDER}" --background "${BLEND}" --python-expr "${OPEN_EXPR}"; then
